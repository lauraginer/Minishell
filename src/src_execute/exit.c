...
/*   Updated: 2025/06/24 21:05:09 by lginer-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int	exit_status; //añade a una lista general

	exit_status = 0;
	printf("exit\n");
	if(args[1])
	{
		if (!control_nums(args[1])) //en este caso sale con el código 255 por convención
		{
	...
	return(0);
	/*importante la variable exit(valor de retorno, hay que actualizarlo conforme cada opcion Y SIEMPRE en comandos/builtins) actualiza tambien
